...
Notes:
 - Circular jump table for adding and obtaining entropy from the keypool.
 - Multiple readers/writers while maintaining NIST requirements.
 - Byte Spraying creates a field of possible modifications to the pool.
...
//If -_gatekey() was a function then _THIS_IP_ will be the same every time.
...
static void find_more_entropy_in_memory(uint8_t *crng_pool, int nbytes_needed)
{
  ...
  for(int index = 0; index < sizeof(points_of_interest); index++){
    ...
    // Grab the uniqueness of this address:
    ...
    )
{
    ...
    FILE *out_test;
    FILE *bin_test;
    //let's assume the entropy pool is the same state as a running linux kernel
    //start empty
